    ...
    def train(self, iterations):
        ...
        for _ in (bar := tqdm(range(int(iterations)), initial=self.iteration, total=self.iteration+iterations)):
            for opt in self.optimizer:
                opt.zero_grad()
            # 这里直接给类传参，会调用__call__或者forward方法
            # 分别采样图的连续形式和离散形式
            ...
            # disc_out["probs"][0, classes] 表示model输出的离散值的概率的classes类，这个if判断了采样图的模型预测是否在预期值取件内
            ...
            # cont_out | self.sampler.to_dict() 看criterion的定义，里面有许多组参数，分别计算损失，后面更新参数
