...
def normalize_adj(adj: object) -> object:
...
class DisNets(nn.Module):
  ...
  #      cur_out = self.input_non_linear(cur_out)
        ...
        graph_embedding = torch.mean(cur_out, 0) ####  for social network, use mean 图嵌入取节点嵌入的平均 size:[1,64]
      #  print(graph_embedding)
        h1 = self.h1_weights(graph_embedding) # size:1x32
        h1 = self.mlp_non_linear(h1) # size:1x32
        logits = self.h2_weights(h1) # size:1x2
    #    logits = h1
        probs =  self.Softmax(logits) # size:1x2

        return logits, probs # 不同类的嵌入，不同类的概率
...
if __name__ == '__main__':
    ...
    path_adj = './MUTAG/MUTAG/MUTAG_A.txt'
    path_labels = './MUTAG/MUTAG/MUTAG_graph_labels.txt'
    path_graph_indicator = './MUTAG/MUTAG/MUTAG_graph_indicator.txt'
    path_labels_node = './MUTAG/MUTAG/MUTAG_node_labels.txt'
