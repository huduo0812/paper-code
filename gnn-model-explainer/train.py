...
# tensorboardX 是一个用于 PyTorch 深度学习框架的第三方库，
# 它提供了将 PyTorch 模型训练过程中的指标和可视化结果记录到 TensorBoard 中的工具。
...
def train_node_classifier(G, labels, model, args, writer=None):
    ...
    for epoch in range(args.num_epochs):
        ...
        loss.backward()
        '''
        这行代码使用了 PyTorch 的 nn.utils.clip_grad_norm 函数，用于梯度裁剪。
        梯度裁剪的目的是防止梯度爆炸的问题，即梯度的值过大，导致参数更新时出现不稳定的情况。

        具体而言，这行代码将模型的参数梯度进行裁剪，确保其总体的 L2 范数不超过 args.clip。args.clip 是一个超参数，表示梯度的阈值。
        如果梯度的 L2 范数超过这个阈值，就对梯度进行缩放，使其不超过阈值。
        '''
        nn.utils.clip_grad_norm(model.parameters(), args.clip)
...
def main():
    ...
    print(path)
    #  SummaryWriter 类创建了一个写入器对象，用于将训练过程中的指标和可视化结果记录到 TensorBoard 中。
    #  以便通过 TensorBoard 进行可视化分析。例如，你可以记录训练损失、准确率、模型参数的分布等信息。
