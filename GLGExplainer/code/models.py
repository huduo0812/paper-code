...
class GLGExplainer(torch.nn.Module):
    ...
    def __init__(self, len_model, le_model, device, hyper_params, classes_names, dataset_name, num_classes):
        ...
        # 原型初始化
        if hyper_params["assign_func"] != "dynamic":
            ...
            self.prototype_vectors = torch.tensor([], device=device)
    ...
    def train_epoch(self, loader, train=True):
        ...
        for data in loader:
            ...
            # TODO: 改成自适应原型数量
            # 原型分配
            prototype_assignements = utils.prototype_assignement(self.hyper["assign_func"], le_embeddings, self.prototype_vectors, temp=1) if train else (
                utils.prototype_assignement("discrete", le_embeddings, self.prototype_vectors, temp=1))
    ...
    def debug_prototypes(self, le_embeddings, prototype_assignements, y):
        ...
        计算原型的损失，并更新原型
        '''
        print(prototype_assignements, y)
    ...
    def iterate(self, train_loader, val_loader, name_wandb="", config_wandb=None, save_metrics=True, plot=False):
        ...
        for epoch in range(1, self.hyper["num_epochs"]):
            ...
            print(f'{epoch:3d}: Loss: {train_metrics["loss"]:.5f}, LEN: {train_metrics["len_loss"]:2f}, Acc: {train_metrics["acc_overall"]:.2f}, '
                  f'V. Acc: {val_metrics["acc_overall"]:.2f}, V. Loss: {val_metrics["loss"]:.5f}, V. LEN {val_metrics["len_loss"]:.3f}')
        ...
        print(f"The number of prototype: {self.prototype_vectors.size()[0]}")
    ...
    def inspect(self, loader, log_wandb=False, plot=True, is_train_set=False):
        ...
        with torch.no_grad():
            ...
            if plot: # plot embedding
                ...
                fig.supxlabel('principal comp. 1', size=10)
                fig.supylabel('principal comp. 2', size=10)
    ...
    def compute_losses(self, le_embeddings, prototype_assignements, total_losses, concept_vector, y_train_1h, le_y):
        ...
        total_losses["len_loss"] += 0 #len_loss.detach()
        ...
        print(f'pro_dis_loss:{total_losses["prototype_distance_loss"]:.4f}, dis_loss:{total_losses["div_loss"]:.4f}')
        ...
        loss = 0 * len_loss + logic_loss   + prototype_distance_loss + r1_loss + r2_loss + concept_entropy_loss + div_loss + distribution_entropy_loss
        total_losses["loss"] += loss.detach()
        return loss , y_pred
...
class preEmbeder(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim1, hidden_dim2, num_classes, device, lr=0.01, dropout=0.1):
        super(preEmbeder, self).__init__()
        # 图卷积层
        ...
        self.mlp = torch.nn.Sequential(
            torch.nn.Linear(hidden_dim2, 128),
            torch.nn.LeakyReLU(),
            torch.nn.Dropout(dropout),
    ...
    def loss(self, output, labels):
        # 交叉熵损失
        loss = torch.nn.CrossEntropyLoss(reduction="mean")(output, labels)
        return loss
    def train_model(self, train_group_loader, val_group_loader, epochs, device):
        ...
        for epoch in range(epochs):
            ...
            for data in train_group_loader:
                # 准备数据
                data = data.to(device)
                self.optimizer.zero_grad()
                # 前向传播
                ...
                epoch_acc += self.compute_accuracy(output, data.y)

            # 计算平均损失和精度
            avg_train_loss = epoch_loss / len(train_group_loader)
            avg_train_acc = epoch_acc / len(train_group_loader)
            ...
            self.eval()  # 设置模型为评估模式
            with torch.no_grad():
                for data in val_group_loader:
                    data = data.to(device)
                    output = self(data)
                    loss = self.loss(output, data.y)
                    val_loss += loss.item()
                    val_acc += self.compute_accuracy(output, data.y)

            # 计算平均验证损失和精度
            avg_val_loss = val_loss / len(val_group_loader)
            avg_val_acc = val_acc / len(val_group_loader)
